...
@push('styles')
<style>
/* ── Reutiliza los mismos estilos de publicaciones ── */
.pub-page { max-width: 1100px; margin: 0 auto; padding: 24px 20px 60px; }
.pub-section-bar {
    display: flex; align-items: center; justify-content: space-between;
    background: #f0fdf4; border-bottom: 2px solid #bbf7d0;
    padding: 10px 16px; margin-bottom: 24px; border-radius: 6px 6px 0 0;
}
.pub-section-title { font-size: 15px; font-weight: 600; color: #14532d; }
.pub-section-actions { display: flex; gap: 10px; align-items: center; }
.btn-refrescar {
...
.btn-refrescar:hover { background: #0e7490; }
.btn-nueva-amb {
    background: #16a34a; color: #fff; border: none;
    padding: 6px 16px; border-radius: 5px; font-size: 13px;
    font-weight: 500; cursor: pointer; transition: background .2s;
}
.btn-nueva-amb:hover { background: #15803d; }

.pub-card {
    border: 1px solid #d1d5db; border-radius: 8px;
    overflow: hidden; margin-bottom: 28px;
    box-shadow: 0 1px 4px rgba(0,0,0,.07); background: #fff;
}
.pub-card-header {
    display: flex; align-items: center; justify-content: space-between;
    background: #e5e7eb; padding: 8px 14px; gap: 10px;
}
.pub-card-titulo {
    font-size: 14px; font-weight: 600; color: #1e293b;
    flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
.pub-card-acciones { display: flex; gap: 6px; flex-shrink: 0; }
...
.pub-viewer-wrap {
    ...
    min-height: 460px; border-radius: 4px; overflow: hidden;
    display: flex; align-items: center; justify-content: center;
}
.pub-viewer-wrap iframe { width: 100%; height: 520px; display: block; border: none; }
.pub-viewer-wrap img    { width: 100%; max-height: 520px; object-fit: contain; display: block; }
.pub-no-preview {
...
.pub-form-group { display: flex; flex-direction: column; gap: 6px; margin-bottom: 14px; }
...
.btn-publicar:hover { background: #15803d; }
.btn-publicar:disabled { background: #86efac; cursor: not-allowed; }
.btn-cerrar-modal {
...
.pub-progress-label { font-size: 11px; color: #6b7280; margin-top: 4px; display: none; }

/* Confirmación */
.pub-confirm-overlay {
    display: none; position: fixed; inset: 0;
    background: rgba(0,0,0,.4); z-index: 9200;
    align-items: center; justify-content: center;
}
.pub-confirm-overlay.active { display: flex; }
.pub-confirm-box {
    background: #fff; border-radius: 10px;
    padding: 28px 28px 20px; width: 360px; max-width: 92vw;
...
.pub-toast {
    ...
    animation: pub-slideIn .3s ease; cursor: pointer;
}
...
<div class="pub-page">
    ...
    <div id="pub-lista">
        ...
        <div class="pub-empty" id="pub-empty-state">
            ...
            <p>No hay estadísticas publicadas aún.</p>
        </div>
        @endforelse
    </div>
...
{{-- ── Modal: Nueva estadística (Admin only) ───────────────────────── --}}
@if($esAdmin)
    ...
    <div class="pub-modal">
        ...
        <div class="pub-modal-body">
            <div class="pub-form-group">
                ...
                <input type="text" id="nf-titulo" placeholder="Título del documento">
            </div>
            <div class="pub-form-group">
                <label>Archivo:</label>
...
<div class="pub-confirm-overlay" id="confirm-overlay">
    <div class="pub-confirm-box">
        <p>¿Eliminar la estadística <strong id="confirm-titulo"></strong>?<br>
...
// ── Modal nueva estadística ───────────────────────────────────────────────
document.getElementById('btn-nueva-estadistica')?.addEventListener('click', () => {
...
function publicar() {
    ...
    formData.append('seccion', 'ambiente');
    ...
    xhr.addEventListener('load', () => {
        if (xhr.status === 200) {
            ...
            if (res.ok) {
                toast('Estadística publicada correctamente.', 'ok');
                cerrarModal();
...
function ejecutarEliminar() {
    ...
    .then(res => {
        if (res.ok) {
            ...
            toast('Estadística eliminada.', 'ok');
        } else {
