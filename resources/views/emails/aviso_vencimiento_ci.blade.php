...
<div class="wrap">
  ...
  <div class="header">
    ...
    <p>Control de Instrumentos — F&amp;C Chile SPA</p>
  </div>

  {{-- Alerta --}}
  @php
    $dias = \Carbon\Carbon::today()->diffInDays($programa->proxima, false);
    $badgeClass = $dias <= 7 ? 'rojo' : '';
  @endphp
  <div class="alert-bar">
    <span class="icon">🔔</span>
    <span class="text">
      El certificado de calibración vence en <strong>{{ $dias }} día{{ $dias !== 1 ? 's' : '' }}</strong>.
      Se requiere acción antes del <strong>{{ $programa->proxima->format('d-m-Y') }}</strong>.
    </span>
  </div>

  {{-- Cuerpo --}}
  <div class="body">
    <p>Estimado/a <strong>{{ $programa->responsable ?: 'Responsable' }}</strong>,</p>
    <p>
    ...
    <span class="dias-badge {{ $badgeClass }}">
      Vence en {{ $dias }} día{{ $dias !== 1 ? 's' : '' }} — {{ $programa->proxima->format('d \d\e F, Y') }}
    </span>

    <div class="card">
      <div class="card-row">
        <span class="label">Equipo / Herramienta</span>
        <span class="value">{{ $programa->descripcion }}</span>
      </div>
      @if($programa->marca || $programa->modelo)
      <div class="card-row">
        <span class="label">Marca / Modelo</span>
        <span class="value">{{ trim($programa->marca . ' ' . $programa->modelo) }}</span>
      </div>
      @endif
      @if($programa->serie)
      <div class="card-row">
        <span class="label">N° Serie</span>
        <span class="value">{{ $programa->serie }}</span>
      </div>
      @endif
      @if($programa->certificado_calibracion)
      <div class="card-row">
        <span class="label">N° Certificado actual</span>
        <span class="value">{{ $programa->certificado_calibracion }}</span>
      </div>
      @endif
      @if($programa->area)
      <div class="card-row">
        <span class="label">Proyecto / Contrato</span>
        <span class="value">{{ $programa->area->descripcion }}</span>
      </div>
      @endif
      <div class="card-row">
        <span class="label">Última calibración</span>
        <span class="value">{{ $programa->ultima?->format('d-m-Y') ?? '—' }}</span>
      </div>
      <div class="card-row">
        <span class="label">Próxima calibración</span>
        <span class="value" style="color:#D97706;font-weight:700">{{ $programa->proxima->format('d-m-Y') }}</span>
      </div>
    </div>
