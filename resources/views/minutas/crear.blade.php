...
<div class="form-body">
    ...
    <form method="POST" action="{{ route('minutas.store') }}">
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px">
            ...
            <button type="button" class="btn-guardar" id="btn-previa-notif">💾 Guardar Minuta</button>
        </div>
...
document.getElementById('btn-previa-notif').addEventListener('click', function () {
    ...
    const form = document.querySelector('form[action="{{ route('minutas.store') }}"]');
...
function guardarSinNotificar() {
    cerrarModalNotif();
    mostrarCargando('Guardando minuta...');
    document.querySelector('form[action="{{ route('minutas.store') }}"]').submit();
}
